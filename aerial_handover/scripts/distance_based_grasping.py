...
class DistanceBasedGrasping:
    def __init__(self):
        ...
        # Pose storage
        self.hand_pose = None
        self.snake_pose = None
        
        # Subscribers
        self.hand_sub = rospy.Subscriber('/hand/mocap/pose', PoseStamped, self.hand_pose_callback)
        self.snake_head_sub = rospy.Subscriber('/snake_head/mocap/pose', PoseStamped, self.snakePoseCb)
    ...
    def snakePoseCb(self, msg):
        """Callback for /snake_head/mocap/pose topic"""
        self.snake_pose = msg
        self.check_distance()
    
    def calculate_distance(self):
        """Calculate Euclidean distance between hand and snake poses"""
        if self.hand_pose is None or self.snake_pose is None:
            return None
        
        hand_pos = self.hand_pose.pose.position
        snake_pos = self.snake_pose.pose.position
        
        distance = np.sqrt(
            (hand_pos.x - snake_pos.x)**2 +
            (hand_pos.y - snake_pos.y)**2 +
            (hand_pos.z - snake_pos.z)**2
        )
        self.snake_head_sub = rospy.Subscriber('/snake_head/mocap/pose', PoseStamped, self.snakePoseCb)
